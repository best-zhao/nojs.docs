 ...
 * 表单验证
 */
define("nojs/module/form", [], function(require, $, ui) {
    var form = function(btn, rules, opt) {
        ...
        this.btn = btn;
        this.rules = rules;
        this.form = opt.form || btn.closest("form");
        //无form标签是，将opt.formSubmit设为false,opt.form指定相应的父容器对象
        if (!this.form.length) {
            return;
        }
        this.item = [];
        //存储所需验证对象
        this.showIco = opt.showIco == false ? false : true;
        //是否显示提示图标
        ...
        //表单提交时事件
        this.onBeforeSubmit = opt.onBeforeSubmit;
        //表单验证前事件
        this.checkMode = opt.checkMode || "submit";
        //验证模式 'blur/keyup'
        this.init();
    };
    form.prototype = {
        init: function(rules) {
            var m, i, j, name, rule, replace, type, _rule, T = this;
            //rules 传入新的规则重新初始化表单				
            this.rules = $.extend(true, {}, rules || this.rules);
            //copy副本
            this.item = [];
            for (i in this.rules) {
                rule = this.rules[i];
        ...
        check: function(m, isSubmit, _rule) {
            ...
            var T = this, n = _rule || m.data("rule"), t = false, v, M, tip = "", opt, s, group, type = this.type(m), state = form.state, reg = form.reg;
            if (!n) {
                return true;
            }
    ...
    form.state = function(m, s, tip) {
		 ...
		 */
        tip = tip || "";
        var wrap = $('<span class="nj_f_tip"><span class="tip_ico"></span><span class="tip_con">' + tip + "</span></span>"), ico, t = wrap.find("span.tip_ico"), offset;
        ...
        remote: function(val, param, element) {
            ...
            $.ajax({
                ...
                success: function(json) {
                    ...
                    if (json.state == 1 || json.status == 1 || param.check && param.check(json)) {
                        //ok
