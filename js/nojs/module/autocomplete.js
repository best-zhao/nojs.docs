 ...
 * 2013-8-29
 */
define("nojs/module/autocomplete", [], function(require, $) {
    ...
    var autoComplete = function(text, opt) {
        ...
        this.state = false;
        //是否有匹配结果
        this.searchOnSelect = opt.searchOnSelect == false ? false : true;
        //选中立即搜索
        this.rewriteOnMove = opt.rewriteOnMove == false ? false : true;
        //键盘上下选择的时候是否重写文本框的值
    ...
    autoComplete.prototype = {
        bind: function() {
            ...
            this.autoComplete.click(function(e) {
                var t = e.target, tag = t.tagName.toLowerCase(), m = $(t);
        ...
        complete: function(v, id) {
            var T = this, val = typeof v == "undefined" ? this.text.val() : v, t = this.autoComplete.find("dt"), html;
            if (this.async == true) {
                //ajax异步模式
        ...
        move: function(d, id) {
            ...
            var list = this.autoComplete.find("dd"), c = list.filter(".current"), prev, next, now;
            if (d == "up") {
                if (!c.length) {
        ...
        search: function() {
            if (this.text.val() != this.text.data("text")) {
                this.complete();
                //防止使用鼠标粘贴时，不能触发keyup事件
                this.showBox("hide");
            }
            var text = this.text.data("text"), //item = $('#'+id),
            v = this.text.val();
            if (!text) {} else {
