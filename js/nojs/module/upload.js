    ...
    upload.prototype = {
        ...
        uploading: function(id) {
            ...
            if (browser["4"]) {
                ...
                var name = "uploadiframe" + +new Date(), callback = "upload" + +new Date(), action = opt.uploader + (opt.uploader.indexOf("?") < 0 ? "?" : "&") + "jsoncallback=" + callback, formData = '<input type="hidden" name="uploadID" value="' + id + '" />';
                for (var i in opt.formData) {
