	...
	return function(id,data,opt){
		...
		(function(){
			...
			box.on( 'click', function(e){
				...
				}else if(tag.hasClass('folder')||tag.hasClass('item')||tag.hasClass('text')||tag.hasClass('line')){//选中
					(!tag.hasClass('item')) && (tag = tag.parent('.item'));
					box.find('.current').removeClass('current');
					tag.addClass('current');
					opt.onSelect&&opt.onSelect.call(arr,tag.attr('id'),tag.attr('href'));//执行事件
				}
				return false;
			})
		})();
