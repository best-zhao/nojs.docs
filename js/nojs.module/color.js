define(function( require, $ ){
	function color(obj){
		...
		function init(){
			var html = [
				'<div id="nj_color">',
					'<div class="mask"><div class="b"></div><div class="c" data="s"></div></div>',
					'<div class="roll"><div class="c" data="r"></div></div>',
					'<div class="r">',
						'<span class="preview"><i></i>颜色</span>',
						'<dl>',
							'<dd><label>H：</label><input name="h" value="0" />度</dd>',
							'<dd><label>S：</label><input name="s" value="0" />%</dd>',
							'<dd><label>B：</label><input name="b" value="0" />%</dd>',
						'</dl>',
						'<dl>',
							'<dd><label>R：</label><input name="_r" value="255" /></dd>',
							'<dd><label>G：</label><input name="_g" value="255" /></dd>',
							'<dd><label>B：</label><input name="_b" value="255" /></dd>',
							'<dd class="color"><label>#</label> <input name="color" value="#ffffff" /></dd>',
						'</dl>',
						'<input type="button" class="ok" value="确定" /> <input type="button" class="close" value="取消" />',
					'</div>',
				'</div>'
			];
			...
			num = {h:text.eq(0),s:text.eq(1),b:text.eq(2),R:text.eq(3),G:text.eq(4),B:text.eq(5),color:text.eq(6)}
		...
		function bind(){
			...
			function move(m,e,ismove){
				var s = !m.hasClass('roll'),
					d = s?maskDrag:rollDrag,
					overflow = s?6:3;
				mouse = {//鼠标位置
					x : e.clientX,
					y : e.clientY
				...
				top = top<0?0:top;
				top = top>255?255:top;
				d.css({'top':top-overflow});
				
				if(s){
					pos.b = 100-top*100/255;//亮度
					left = mouse.x+$(window).scrollLeft()-m.offset().left;
					left = left<0?0:left;
					left = left>255?255:left;
					d.css({'left':left-overflow});
					pos.s = left*100/255;//饱和度
				}else{
		...
		function reset(_color){
			//初始化时重置为默认颜色,必须为16进制
			if(!_color||!/^#([0-9a-fA-f]{3}|[0-9a-fA-f]{6})$/.test(_color)){return;}
			var rgb = color.HexToRgb(_color),
			...
			maskDrag.css({'top':255-hsb.B*255/100-6,'left':hsb.S*255/100-6});
			rollDrag.css({'top':255-hsb.H*255/360-3});
		}
		function add(obj){
			/*
			 * 添加一组触发对象
