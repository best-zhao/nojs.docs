...
define(function( require, $, ui ){
	...
	function lightbox(id, options){
		...
		this.item = this.box.find(this.tag);
		this.length = this.item.length;
		this.showInfo = options.showInfo==false ? false : true;
	...
	lightbox.prototype = {
		...
		bind : function(){
			...
			if( this.showInfo ){//显示图片简介及缩略图等信息
				...
				$.each( this.item, function(i,m){
					img = m.tagName.toLowerCase()=='img' ? $(m) : $(m).find('img');
					if( img.length ){
						img = img.clone().attr({'style':''});
						pic.append(img);
					}
				})
				pic.width( img.outerWidth()*this.length );
				this.number = this.info.find('.num');
		...
		load : function(index, resize){
			...
			if( this.showInfo ){
				small = this.info.find('.wrap img');				
				this.info.find('.wrap').stop().animate({
					'scrollLeft' : small.outerWidth()*(index-2)
				},200);
				this.number.find('em').text( index+1 );
				this.info.find('.title').html( obj.attr('data-title') );
				this.info.find('.desc').html( obj.attr('data-desc') );
			}
