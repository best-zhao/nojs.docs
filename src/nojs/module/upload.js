...
define(function(require,$){
	...
	function upload( button, options ){
		...
		var opt = $.extend({
			...
			'name' : button[0].name || 'file',	//input文件的name值
		...
		this.tip = typeof opt.tip=='function' ? opt.tip : function(c){alert(c)};
		if( opt.dragUp && opt.dragUp.length ){//拖拽上传
			upload.dragUp(this);
		}
		this.init();
	}
	
	upload.config = {};							//全局配置, options可覆盖
	
	upload.formatSize = function(size){			//格式化文件大小
		if( !size ){
			return null;
		}
		size = size/1024;
