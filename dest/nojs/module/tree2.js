 ...
 * 目录树
 */
define("nojs/module/tree2", [], function(require, $) {
    return function(id, data, opt) {
		 ...
		 * @data:json数组Array
		 */
        if (!id || $.type(data) !== "array" || !data.length) {
            return;
        }
        opt = opt || {};
        var box = $("#" + id), item = "", c = 0, arr = {
            init: init,
            box: box,
            setNode: setNode,
            length: 0,
        ...
        function init(Data) {
            ...
            function read(data, level) {
                var len = data.length, i, j, now, m, link, line, id, open;
                item += "<ul>";
                for (i = 0, c = level; i < len; i++) {
                    ...
                    if (m && m["text"]) {
                        ...
                        item += '<a class="item" href="' + link + '" reallink="' + link + '" ' + id + " " + open + ">" + line + '<i class="ico"></i><i class="folder"></i><span class="text">' + m["text"] + "</span></a>";
                        arr.length++;
                        for (j in m) {
                            now = m[j];
            ...
            addClass();
            (function() {
                var node = box.find("a.item").not(".no_child");
                //包含子节点
                //展开全部
                if (opt.openAll) {
                    box.find("ul ul").show();
        ...
        function setNode(ID, by) {
            ...
            function set() {
                ...
                opt.onSelect && opt.onSelect.call(arr, ID, node.attr("href"));
                //执行事件
                return false;
            }
            function s(i) {
        ...
        //节点添加特殊类
        function addClass() {
            var list = box.find(".item"), i, j, n = list.length, m, li, level;
            for (i = 0; i < n; i++) {
                m = list.eq(i);
        ...
        (function() {
            ...
            box.on("click", function(e) {
                ...
                } else if (tag.hasClass("folder") || tag.hasClass("item") || tag.hasClass("text") || tag.hasClass("line")) {
                    //选中
                    !tag.hasClass("item") && (tag = tag.parent(".item"));
                    box.find(".current").removeClass("current");
                    tag.addClass("current");
                    opt.onSelect && opt.onSelect.call(arr, tag.attr("id"), tag.attr("href"));
                }
                return false;
            });
        })();
